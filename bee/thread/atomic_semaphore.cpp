...
#else
    // TODO ulock_wait/umtx_op
#    define BEE_NO_KERNEL_WAIT
#endif
    constexpr uint32_t SEM_FALSE = UINT32_C(0);
    constexpr uint32_t SEM_TRUE  = UINT32_C(1);
    ...
    void atomic_semaphore::release() noexcept {
        ...
        const auto* ptr = &v;
        v.store(SEM_TRUE);
        kernel_wake((const uint32_t*)ptr, false);
#else
        v.store(SEM_TRUE);
#endif
    }
    ...
    void atomic_semaphore::acquire() noexcept {
        ...
        for (;;) {
            if (v.exchange(SEM_FALSE) == SEM_TRUE)
                return;
            kernel_wait((const uint32_t*)&v, SEM_FALSE);
        }
#else
        for (int i = 0; i < 64; ++i) {
            if (v.exchange(SEM_FALSE) == SEM_TRUE)
