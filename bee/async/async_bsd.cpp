...
namespace bee::async {
    ...
    int async::poll(const span<io_completion>& completions) {
        ...
        count += drain_kqueue(m_kqfd, span<io_completion>(completions.data() + count, completions.size() - count), 0, m_pending_ops);
        return count;
    }
    ...
    int async::wait(const span<io_completion>& completions, int timeout) {
        ...
        count += drain_kqueue(m_kqfd, completions, timeout, m_pending_ops);
        return count;
    }
